...
const Component: React.FC<IProfileContainerProps> = ({ history, match, post, profile: profileStore }) => {
  ...
  if (profile?.username === params.username || `${profile?.username}/` === params.username) {
    ...
    return <ProfileWrapper history={history} session={session} profile={profileStore} username={params.username} post={post} />
  }
}
