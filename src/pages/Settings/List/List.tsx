      ...
      {listData.map((item, index) => {
        return (
          <li key={index} className={styles['list-item']}>
            <Link className={styles['list-item-link']} to={item.link}>
              {item.title}
            </Link>
          </li>
        );
...
export const EditProfileListItem: React.FC<EditProfileListItemProps> = ({
  userData,
  openModal,
  openSexModal,
}) => {
  return (
    <>
      <li
        ...
        onClick={() => openModal({ isVisible: true, modalName: 'fullName' })}
      >
      ...
      <li
        className={styles['list-item']}
        onClick={() => openModal({ isVisible: true, modalName: 'userName' })}
      >
        <label className={styles['list-item-link']}>Usename</label>
        <button className={styles['list-item-btn']}>
          {userData.userName}
          <ArrowNext />
        </button>
      ...
      <li
        className={styles['list-item']}
        onClick={() => openModal({ isVisible: true, modalName: 'birthDay' })}
      >
      ...
      <li className={styles['list-item']} onClick={() => openSexModal(true)}>
        <label className={styles['list-item-link']}>Sex</label>
        <button className={styles['list-item-btn']}>
          {userData.gender}
          <ArrowNext />
        </button>
...
const List: React.FC = ({ children }) => {
  return (
    <div className={styles['list-container']}>
      <ul className={styles['list-items']}>{children}</ul>
    </div>
  );
};
